import { useDispatch, useSelector } from "react-redux";
import SidebarContent from "./SidebarContent";
import { selectShowSidebar, toggleSidebar } from "../../Redux/reducers/interfaceSettingsSlice";

export default function SidebarDrawer({ localState, handleNewChat }: { localState: any, handleNewChat: () => void }) {

  const dispatch = useDispatch();
  const showSidebar = useSelector(selectShowSidebar);
  console.log("Rendering SidebarDrawer, isOpen:", showSidebar);
  return (
    <>
      {/* Backdrop */}
      {showSidebar && (
        <div
          className="fixed inset-0 z-30 backdrop-blur-sm bg-black/30 dark:bg-gray/30 transition-opacity"
          onClick={() => dispatch(toggleSidebar())} // clicking backdrop closes drawer
        />
      )}

      {/* Drawer */}
      {/* height is: screenheight-header_size which is h-14 */}
      <div
        id="sidebardrawer"
        className={`fixed top-[3.5rem] md:top-0 left-0 pt-4 z-40 h-[calc(100vh-3.5rem)] md:h-screen overflow-y-auto transition-transform duration-300
          ${showSidebar ? "translate-x-0" : "-translate-x-full"} 
          bg-white dark:bg-bg_secondary_dark shadow-lg`}
      >
        <SidebarContent localState={localState} handleNewChat={handleNewChat} />
      </div>
    </>
  );
}
