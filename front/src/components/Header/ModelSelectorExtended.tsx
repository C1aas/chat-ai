...
export default function ModelSelectorSimple({modelsList}: {modelsList: ExtendedModelInfo[]}) {
    ...
    return (
      ...
      >
        <div className="flex justify-between md:flex-row flex-col ">
          ...
          <div className="grid grid-cols-[6rem_6rem_6rem_4.5rem] items-center gap-2">
  ...
  const GridElement = memo(({idx, model, onClick} : {idx: number, model: ExtendedModelInfo, onClick: () => void}) => {
    return (
      <div 
        onClick={onClick}
        tabIndex={idx} data-index={idx} data-id={model.id}
        className="group rounded-2xl border border-slate-200 bg-white p-3 h-full hover:shadow-md transition dark:border-slate-700 dark:bg-slate-800 focus-within:ring-2 focus-within:ring-indigo-500 flex flex-col justify-between"
      >
      ...
      <div className={`${dropdownOpen ? "" : "hidden"} absolute z-50 mt-1 w-full rounded-2xl border border-slate-200 bg-white shadow-2xl  pb-4`}>
        ...
        <div className="text-sm flex items-center gap-2 p-2 border-b border-slate-100 to-white">
          ...
          <Menu as="div" className="relative inline-block text-left ">
            ...
            <MenuButton className="h-9 inline-flex w-full justify-between items-center rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-600 shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-indigo-500/40">
            ...
            <MenuItems className="absolute right-0 mt-2 w-40 origin-top-right rounded-xl bg-white shadow-lg ring-1 ring-black/5 focus:outline-none z-50">
              ...
              <MenuItem>
                ...
                <label className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 cursor-pointer">
                  <input type="checkbox" className="filter-out" data-out="thought" /> Reasoning / Thinking
                </label>
              </MenuItem>
              <MenuItem>
                <label className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-slate-50 cursor-pointer">
